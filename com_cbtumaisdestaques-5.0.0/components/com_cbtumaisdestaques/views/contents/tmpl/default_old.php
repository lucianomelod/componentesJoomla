...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post"
		...
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->state)) : ?>
					...
					<td class="center">
	<!--<a class="btn btn-micro <?php echo $class; ?>" href="<?php echo ($canChange) ? JRoute::_('index.php?option=com_cbtumaisdestaques&task=content.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
	<?php if ($item->state == 1): ?>
	...
	<?php if ($canCreate) : ?>
		<a href="<?php echo JRoute::_('index.php?option=com_cbtumaisdestaques&task=contentform.edit&id=0', false, 0); ?>"
